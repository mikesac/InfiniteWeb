<%@page import="org.infinite.web.PagesCst"%>
<html>
<head>

<title>Welcome</title>
</head>
<body>


<table cellspacing="10px" cellpadding="10px">
	<tr>
		<td>
		<div class="ui-dialog ui-widget ui-widget-content ui-corner-all" style="width:250px">
			<div class="ui-dialog-titlebar ui-widget-header ui-corner-all ui-helper-clearfix">
				<span id="ui-dialog-title-dialog" class="ui-dialog-title">Play Game</span>
				<span class="ui-dialog-titlebar-close ui-corner-all">
					<span class="ui-icon ui-icon-person">person</span>
				</span>
			</div>
			<div style="height: 200px;text-align:center;" class="ui-dialog-content ui-widget-content" id="dialog">
			<a href="<%=request.getContextPath() + PagesCst.PAGE_CHARACTER%>">
			<img border=0 src="./imgs/web/home/play.gif" alt="play" title="play" />
			</a>
		</div>
		</div>
		</td>
		<td>
		<div class="ui-dialog ui-widget ui-widget-content ui-corner-all" style="width:250px">
		<div
			class="ui-dialog-titlebar ui-widget-header ui-corner-all ui-helper-clearfix">
		<span id="ui-dialog-title-dialog" class="ui-dialog-title">Expand World</span>
		 <span class="ui-dialog-titlebar-close ui-corner-all"><span
			class="ui-icon ui-icon-image">image</span></span></div>
		<div style="height: 200px;text-align:center;"	class="ui-dialog-content ui-widget-content" id="dialog">
			<a href="<%=request.getContextPath() + PagesCst.ADMIN_HOME%>">
			<img border="0" src="./imgs/web/home/expand.gif" alt="expand" title="expand" />
			</a>
		</div>
		</div>
		</td>
		</tr>
		<tr>
			<td>
				<div class="ui-dialog ui-widget ui-widget-content ui-corner-all" style="width:250px">
					<div class="ui-dialog-titlebar ui-widget-header ui-corner-all ui-helper-clearfix">
					<span id="ui-dialog-title-dialog" class="ui-dialog-title">Register</span> 
					<span class="ui-dialog-titlebar-close ui-corner-all"><span
						class="ui-icon ui-icon-home">home</span></span></div>
					<div style="height: 200px;text-align:center;" class="ui-dialog-content ui-widget-content" >
						<a href="<%=request.getContextPath() + PagesCst.PAGE_REGISTER%>">
							<img border="0" src="./imgs/web/home/register.gif" alt="register" title="register" />
						</a>
					</div>
				</div>
			</td>
		
		<td>
		<div class="ui-dialog ui-widget ui-widget-content ui-corner-all" style="width:250px">
		<div
			class="ui-dialog-titlebar ui-widget-header ui-corner-all ui-helper-clearfix">
		<span id="ui-dialog-title-dialog" class="ui-dialog-title">Contribute to Project</span> 
		<span class="ui-dialog-titlebar-close ui-corner-all"><span
			class="ui-icon ui-icon-lightbulb">home</span></span></div>
		<div style="height: 200px;text-align:center;" class="ui-dialog-content ui-widget-content" id="dialog">
		<a href="http://github.com/mikesac/infiniteworld">
			<img border="0" src="./imgs/web/home/logo.gif" alt="home" title="home" />
		</a>
		</div>
		</div>
		</td>
	</tr>
</table>






</body>
</html>