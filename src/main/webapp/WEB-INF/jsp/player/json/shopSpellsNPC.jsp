...
<%
	...
	ArrayList<Spell> list = 	(ArrayList<Spell>)Manager.listByQuery("from "+Spell.class.getName() + " i where i.level > '0' and i.level >= '"+ (npc.getLevel()-1) + "' and i.level <= '"+ (npc.getLevel()+1)+ "'");
    ...
    "rows":[
    	<% for(int i=0;i<list.size();i++){
        	...
        	"data":[
        		...
        		<i style='font-size:xx-small;'><%=list.get(i).getDescription()%></i><br/>\
        		...
        		&nbsp;&nbsp;Level:\
        		<%=(list.get(i).getLevel()>c.getLevel())?"<span style='color:red'>":"<span style='color:green'>"%>\
        		<%=list.get(i).getLevel()%>\
        		</span>\
        		"
        		,
